---
title: Affiliations
menu:
  footer_about:
    name: Affiliations
    weight: 3
layout: about_navbar
---

## Affiliations and partners

{{< organization name="Institute of Molecular Genetics of the CAS (IMG)" img="/images/IMG.png" >}}
[www.img.cas.cz](https://www.img.cas.cz) — the lead institution providing the portal's content and analyses.
{{< /organization >}}

{{< organization name="ELIXIR-CZ" img="/images/elixir_logo.png" >}}
The Czech national node of the European [ELIXIR](https://elixir-europe.org) infrastructure, hosting the ELIXIR IMPACT2 project.
{{< /organization >}}

{{< organization name="SIB — Swiss Institute of Bioinformatics" img="/images/SIB.png" >}}
[www.sib.swiss](https://www.sib.swiss) — the Swiss partner of the Czech–Swiss ELIXIR IMPACT2 project.
{{< /organization >}}

{{< organization name="UCT Prague" img="/images/VSCHT.png" >}}
[www.vscht.cz](https://www.vscht.cz) — project partner, collaboration on data analysis.
{{< /organization >}}

{{< organization name="FIT CTU in Prague" img="/images/FIT_CVUT.svg" >}}
[fit.cvut.cz](https://fit.cvut.cz) — Faculty of Information Technology, Czech Technical University in Prague; co-authoring institution.
{{< /organization >}}

{{< organization name="UJEP — Jan Evangelista Purkyně University in Ústí nad Labem" img="/images/UJEP.png" >}}
[www.ujep.cz](https://www.ujep.cz) — co-authoring institution for the situation reports.
{{< /organization >}}

{{< organization name="CESNET" img="/images/CESNET.svg" >}}
[www.cesnet.cz](https://www.cesnet.cz) — operates the portal infrastructure (e-INFRA CZ virtual server).
{{< /organization >}}
